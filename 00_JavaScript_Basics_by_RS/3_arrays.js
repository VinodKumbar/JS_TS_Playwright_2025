...
let expenses = [25, 50, 75, 100, 150]

let sum = 0;
for (let x =0; x<expenses.length; x++){
    sum += expenses[x]; // Calculating the sum of all expenses
}
console.log(sum, "is sum of expenses")

let highest = expenses[0];
let lowest = expenses[0];

for(let y=1; y<expenses.length;y++){
    if(expenses[y]>highest) highest = expenses[y];
    if(expenses[y]<lowest) lowest = expenses[y]
}
...
let studentNames = ["Chris", "John", "Robert", "Mary"]
studentNames.unshift("Gal")
studentNames.pop()
studentNames.sort()
...
let productPrices = [10, 20, 30, 40, 50, 100, 500, 1000, 10000]

let discountedPrices = productPrices.map((productPrices) => productPrices  * 0.9)
console.log("After applying 10% discount on the productPrices " + discountedPrices)

let affordableProducts = discountedPrices.filter((discountedPrices) => discountedPrices < 50)
console.log("List of Product which are below $50 are as follows : " + affordableProducts)

let totalCostofallItems = affordableProducts.reduce((sum, affordableProducts)=> sum + affordableProducts, 0)
console.log(totalCostofallItems, "is the sum of all items")